...
def getNodeValue(node):
    ...
    if node.header[0] == 0: 
        ...
        for m in node.metadata: 
            # maybe one would get a minor speedup from caching the results for child nodes 
            # and calling them if their result is needed again, but this would require a lot of memory, would  
            # make the code more complicated and is not needed here.
            if m > 0 and m <= len(node.children): 
                result += getNodeValue(node.children[m-1])
