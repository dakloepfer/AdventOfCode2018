...
with open("/Users/DominikKloepfer/Documents/workspace/Personal_Projects/Advent of Code 2018/Day 15/test2.txt") as input: 
...
for row in range(len(data)):
    for column in range(len(data[row])):
        if data[row][column] == '.':
            ...
            elves.append(Combatant(row, column, 200, 3, True))
...
while True:
    ...
    for c in range(len(all_combatants)):
        ...
        if len(elves) == 0 or len(goblins) == 0:
            ...
            survivors = elves + goblins
            total_hp_left = sum([s.hp for s in survivors])
            print('Outcome:', total_hp_left * turn)

            for e in elves:
                print('elf', e.row, e.column, e.hp)
            for g in goblins:
                print('goblin', g.row, g.column, g.hp)
            sys.exit()
