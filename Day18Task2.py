...
while m < 1000000000 and not repeated:
    computeOneMinute()
    new_tuple_area = tuple([tuple(row) for row in current_area])
    m += 1

    if new_tuple_area in past_configurations:
        repeated = True
    else:
        past_configurations[new_tuple_area] = m
    print(m)

if m == 1000000000: print('Resource Value after computing everything', calculateResourceValue())
else:
    prev_m = past_configurations[new_tuple_area]
    delta_m = m - prev_m

    remaining_time = (1000000000 - m) % delta_m

    for t in range(remaining_time):
        computeOneMinute()
    
    print('Derived resource value of', calculateResourceValue())